...
DOMAIN = "https://ultramarine-linux.org/"
DOWNLOAD_PAGE_URL = f"https://images.fyralabs.com/isos/ultramarine"
FILE_NAME = "/[[VER]]/Ultramarine-[[EDITION]]-[[VER]]-Live-x86_64-.iso"


class Ultramarine(GenericUpdater):
    """
    A class representing an updater for Ultramarine.
    ...
    def _get_download_link(self) -> str:
        ...
        return f"https://images.fyralabs.com/isos/ultramarine/{latest_version[0]}/ultramarine-{self.edition}-{'.'+latest_version[2] if len(latest_version)>2 else ''}-Live-x86_64.iso"

    def check_integrity(self) -> bool:
        latest_version = self._get_latest_version()
        sha256_url = f"https://images.fyralabs.com/isos/ultramarine/{latest_version[0]}/ultramarine-{self.edition}-{latest_version[0]}-{latest_version[1]}{'.'+latest_version[2] if len(latest_version)>2 else ''}-live-x86_64.iso.sha256sum"

        sha256_sums = requests.get(sha256_url).text

        sha256_sum = parse_hash(sha256_sums, [f"SHA256 (Ultramarine-{self.edition}"], -1)
